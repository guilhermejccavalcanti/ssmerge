...
public class AbstractFSTParser {
 ...
 protected static class FSTInfo {
  ...
  public String toString() {
   return "[" + type + " - " + exportedName + " - " + exportedComposition + "]";
  }
 }
 ...
 protected FSTInfo productionEndNonTerminal(String type, String namePattern, String exportNamePattern) {
  ...
  String exportName = applyReplacements(exportNamePattern, c.nameReplacements, FSTInfoType.NAME);
  type = applyReplacements(type, c.nameReplacements, FSTInfoType.NAME);

  if (!cc().isInTerminal) {
   String name = namePattern.equals(exportNamePattern) ? exportName : applyReplacements(namePattern, c.nameReplacements,FSTInfoType.NAME);
 ...
 private String applyReplacements(String name, List < Replacement > replList,FSTInfoType type) {
  ...
  for (Replacement replacement: replList) {
   ...
   if (listStart >= 0) {
    ...
    name = name.replace(listName, value + (sepChar == '~' ? "" : sepChar) + listName);
   } else {
    // no list
 ...
 protected FSTInfo productionEndTerminal(String type, String namePattern,
  ...
  return productionEndTerminal(type, namePattern, exportNamePattern,compositionMechanism, FSTTerminal.defaultMergingMechanism,first, last);
 }
 ...
 protected FSTInfo productionEndTerminal(String type, String namePattern,
  ...
  c.nameReplacements.add(new Replacement("TOSTRING", new FSTInfo(type,stripWhitespace(body))));

  String exportName = applyReplacements(exportNamePattern,c.nameReplacements, FSTInfoType.NAME);
  type = applyReplacements(type, c.nameReplacements, FSTInfoType.NAME);
  compositionMechanism = applyReplacements(compositionMechanism,c.nameReplacements, FSTInfoType.COMPOSE_OR_MERGE);
  mergingMechanism = applyReplacements(mergingMechanism, c.nameReplacements, FSTInfoType.COMPOSE_OR_MERGE);
  ...
  if (!c.isInTerminal) {
   ...
   if (first != null) {
    /**
     * do not remove parameters beginLine and endLine, this is
     * necessary @ FeatureIDE
     **/
    cc().children.add(new FSTTerminal(type, name, body, prefix,compositionMechanism, mergingMechanism,first.beginLine, last.endLine));
   }
  }
 ...
 private String applyComposeReplacements(String compositionMechanism) {
  if (compositionMechanism.length() > 0 && compositionMechanism.charAt(0) == '{') {
   ...
   for (String productionKey: composeReplacements.keySet()) {
    compositionMechanism = compositionMechanism.replace("{" + productionKey + "}", composeReplacements.get(productionKey));
   }
  }
