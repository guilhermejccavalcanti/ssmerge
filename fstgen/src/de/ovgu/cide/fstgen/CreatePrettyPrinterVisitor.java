...
public class CreatePrettyPrinterVisitor extends NCreateFileVisitor {
	...
	public boolean visit(NChoice c) {
		...
		if (!JavaCCPrintVisitor.getFSTType(c).equals(
				'"' + c.getParent().getName() + '"'))
		...
		if (c.isList()) {
			println("Iterator<FSTNode> listElements = getChildren(nonTerminal, \""
	...
	private void printHead() {
		...
		println("public class SimplePrintVisitor extends AbstractFSTPrintVisitor  {",
				0);
	...
	private void printTail() {
		println("throw new RuntimeException(\"Unknown Non Terminal in FST \"+nonTerminal);",
				2);
		println("}", 1);

		// subtype function
		println("protected boolean isSubtype(String type, String expectedType) {",
				1);
