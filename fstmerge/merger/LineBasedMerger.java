...
import merger.handlers.ImplementsUnion;
...
public class LineBasedMerger implements MergerInterface {
	...
	public void merge(FSTTerminal node) throws ContentMergeException {
		...
		if(!(node.getType().contains("-Content") ||	node.getMergingMechanism().equals("LineBased"))) {
			...
			if(node.getType().equals("ImplementsList")){
				(new ImplementsUnion()).compose(tokens[0], tokens[1], tokens[2], node);
			}
		...
		try {
			...
			if(System.getProperty("os.name").contains("Windows"))
				...
				mergeCmd = "C:/KDiff3/bin/diff3.exe -m -E " + "\"" + fileVar1.getPath() + "\"" + " " + "\"" + fileBase.getPath() + "\"" + " " + "\"" + fileVar2.getPath() + "\"";
			else
